            ...
            {loading ? (
              ...
              >
                {[0, 1, 2, 3, 4, 5, 6, 7].map((idx) => (
                  <div key={idx} className="flex flex-col w-full">
                    ...
                    {/* Skeleton Card */}
                    <div className="relative flex flex-col items-center justify-center p-2.5 sm:p-3 md:p-3.5 border border-white/15 bg-white/[0.04] backdrop-blur-md rounded-tl-2xl rounded-br-2xl rounded-tr-md rounded-bl-md sm:rounded-tl-3xl sm:rounded-br-3xl overflow-hidden w-full">
                      ...
                      {/* Member Photo Frame Placeholder matching actual photo ratio */}
                      <div className="w-full aspect-[1536/1285] rounded-xl sm:rounded-2xl border border-white/10 bg-zinc-950/80 p-1 flex items-center justify-center">
              ...
              >
                {team.map((member, index) => {
                  ...
                  const ratio = aspectRatios[memberKey] || (1536 / 1285);
                  return (
                    <motion.div
                      key={memberKey || index}
                    ...
                    >
                      ...
                      <motion.div
                        ...
                        className="flex flex-col items-center justify-center p-2.5 sm:p-3 md:p-3.5 group cursor-pointer transition-all duration-300 border border-white/20 bg-white/[0.04] backdrop-blur-md rounded-tl-2xl rounded-br-2xl rounded-tr-md rounded-bl-md sm:rounded-tl-3xl sm:rounded-br-3xl hover:border-ted-red/60 hover:bg-white/[0.07] shadow-[0_4px_30px_rgba(0,0,0,0.3)] hover:shadow-[0_0_30px_rgba(235,0,40,0.18)] w-full"
                      >
                        {/* Member Photo Frame - automatically adapts box size and shape based on image */}
                        <div 
                          className="w-full rounded-xl sm:rounded-2xl overflow-hidden border border-white/15 bg-zinc-950/60 p-1 flex items-center justify-center shadow-inner"
                            ...
                            onLoad={(e) => {
                              ...
                              if (naturalWidth && naturalHeight) {
                                ...
                                setAspectRatios((prev) => {
                                  if (prev[memberKey] === calculatedRatio) return prev;
                                  return { ...prev, [memberKey]: calculatedRatio };
                                });
                              }
                            }}
                            className="w-full h-full object-contain rounded-lg sm:rounded-xl transition-all duration-500 group-hover:scale-105" 
                          />
                        </div>
      ...
      <AnimatePresence>
        {activeMember && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
            ...
            {/* Modal Dialog with exact asymmetric corners */}
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              transition={{ type: "spring", duration: 0.5 }}
              className="relative w-full max-w-lg bg-ted-dark-gray border border-white/10 rounded-tl-[3.5rem] rounded-br-[3.5rem] rounded-tr-xl rounded-bl-xl shadow-2xl z-10 overflow-hidden flex flex-col items-center cursor-default max-h-[90vh]"
            >
              {/* Decorative Corner Tech Brackets - Pinned to Dialog Frame */}
              <div className="absolute top-0 right-0 w-4 h-4 border-t-2 border-r-2 border-ted-red pointer-events-none z-20" />
              <div className="absolute bottom-0 left-0 w-4 h-4 border-b-2 border-l-2 border-ted-red pointer-events-none z-20" />

              {/* Pinned Close Button */}
              <button
                onClick={() => setActiveMember(null)}
                className="absolute top-4 right-4 text-white/50 hover:text-white transition-colors cursor-pointer w-8 h-8 flex items-center justify-center z-30 bg-black/50 hover:bg-white/15 rounded-full backdrop-blur-md"
                title="Close"
              >
              ...
              <div className="w-full overflow-y-auto scrollbar-none p-8 md:p-10 flex flex-col items-center text-center">
                {/* Role Badge */}
                ...
                {/* Member Photo */}
                <div 
                  className="w-full max-w-[280px] sm:max-w-[340px] rounded-2xl overflow-hidden border border-white/20 shadow-lg mb-2 bg-black/40 p-1 flex items-center justify-center shrink-0"
                  style={{
                    aspectRatio: aspectRatios[activeMember.id || activeMember.name] || (1536 / 1285)
                  }}
                >
                  <img 
                ...
                <button
                  onClick={() => setActiveMember(null)}
                  className="px-8 py-3.5 bg-ted-red text-white font-black rounded-2xl text-[11px] shadow-[0_0_20px_rgba(235,0,40,0.25)] hover:shadow-[0_0_30px_rgba(235,0,40,0.45)] hover:bg-white hover:text-ted-red transition-all duration-150 uppercase tracking-widest cursor-pointer mt-2 shrink-0"
                >
                  Back to Team
                </button>
              </div>
