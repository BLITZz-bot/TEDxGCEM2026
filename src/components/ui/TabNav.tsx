...
export default function TabNav({ activeTab, onTabChange }: TabNavProps) {
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex justify-center p-6 bg-black/50 backdrop-blur-md">
      <div className="flex flex-wrap items-center justify-center gap-1 sm:gap-2 bg-ted-dark-gray/80 p-1.5 rounded-3xl sm:rounded-full border border-white/10 max-w-full overflow-x-auto no-scrollbar">
        {tabs.map((tab) => (
          <button
            key={tab.id}
